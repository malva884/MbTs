...
class QtCprTestController extends Controller
{
    public function list(Request $request)
    {
        ...
        $objs = DB::table('qt_cpr_tests')->select('qt_cpr_tests.*','qt_categories.categoria')
            ->leftJoin('qt_categories','qt_categories.id','qt_cpr_tests.tipo')
    ...
    public function get_prove(Request $request, $ol)
    {
        ...
        $objs = DB::table('qt_cpr_tests')->select('qt_cpr_tests.*','qt_categories.categoria')
            ->join('qt_categories','qt_categories.id','qt_cpr_tests.tipo')
            ->where('ol', $ol)
            ->get();
    ...
    {
        $obj =  DB::table('qt_cpr_tests')->select('qt_cpr_tests.*','qt_categories.categoria')
            ->join('qt_categories','qt_categories.id','qt_cpr_tests.tipo')
            ->where('qt_cpr_tests.id',$id)->first();
    ...
    {
        if (empty($path)) {
            Log::channel('stderr')->error("QtCprTest saveFile: path cartella Google Drive non valido o vuoto!");
            return false;
        }
        ...
        if (!empty($file)) {
            ...
            if (!$tmpFileObject = $this->validateBase64($base64Image, ['png', 'jpg', 'jpeg', 'HEIC', 'pdf', 'docx', 'xls', 'xlsx'])) {
                Log::channel('stderr')->error("QtCprTest saveFile: formato base64 o mime-type non valido per estensione .{$ext_file}");
                return false;
            }
            ...
            if (!$fileDrive) {
                Log::channel('stderr')->error("QtCprTest saveFile: GoogleDrive::add_file fallito per '{$filename}' nella cartella '{$path}'");
            } else {
                Log::channel('stderr')->info("QtCprTest saveFile: file caricato con successo su Google Drive: '{$filename}' (ID: {$fileDrive})");
            }
