...
class MenceInTransitoCalcoloDistanzaKm implements ShouldQueue
{
    ...
    {
       $rows = DB::table('fi_goods_transit_heads')->where('head',$this->id)->get();
       foreach ($rows as $row){
           $coordinates = null;
           ...
           DB::table('fi_goods_transit_rows')->where('id', '=', $row->id)->update([
