...
const loadItems = async () => {
  ...
  const { data: resultData } = await useApi<any>(createUrl('/pl/typology/list', {
    query: {
      page: page.value,
...
const headers = computed(() => [
  { title: t('Label.Gruppo'), key: 'tipologia' },
...
const save = async () => {
  if (editedItem.value.tipologia) {
    let path = '/pl/typology/store/'
    if (editedItem.value.id)
      path = `/pl/typology/update/${editedItem.value.id}`
...
<template>
  ...
  >
    <AppCardActions
      ...
      :title="editedItem.id ? `${$t('Label.Modifica')} Tipologia` : `${$t('Label.Nuova')} Tipologia`"
      no-actions
    >
      <VCard>
        <VCardText>
          <VContainer>
            ...
            >
              <VRow>
                ...
                <VCol cols="12">
                  <AppTextField
                    v-model="editedItem.icona"
                    :rules="[requiredValidator]"
                    :label="$t('Label.Icona')"
                    :placeholder="$t('Label.Icona')"
                  />
                </VCol>
                <VCol
                  cols="12"
