...
const loadItems = async () => {
  ...
  const { data: resultData, error } = await useApi<any>(createUrl('/fibra_tipologia/list', {
    query: {
      page: page.value,
...
const save = async () => {
  if (editedItem.value.nome) {
    let path = '/fibra_tipologia/store/'
    if (editedItem.value.id)
      path = `/fibra_tipologia/update/${editedItem.value.id}`
...
<template>
  ...
  >
    <AppCardActions
      ...
      :title="editedItem.id ? `${$t('Label.Modifica')} Topologa Fibra` : `${$t('Label.Nuova')} Topologa Fibra`"
      no-actions
    >
      <VCard>
        <VCardText>
              ...
              <VRow>
                ...
                <VCol cols="12">
                  <AppTextField
                    ...
                    :label="$t('Label.Topologia Fibra')"
                    :placeholder="$t('Label.Topologia Fibra')"
                  />
                </VCol>
