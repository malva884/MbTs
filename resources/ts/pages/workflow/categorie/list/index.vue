...
const loadItems = async () => {
  ...
  const { data: resultData } = await useApi<any>(createUrl('/workflow/categorie/', {
    query: {
      page: page.value,
      itemsPerPage: itemsPerPage.value,
      sortBy: sortBy.value,
      orderBy: orderBy.value,
      ruolo: categoriaFilter.value,
...
const save = async () => {
  if (editedItem.value.categoria && editedItem.value.model) {
    let path = '/workflow/categorie/store/'
    if (editedItem.value.id)
      path = `/workflow/categorie/update/${editedItem.value.id}`
          ...
          <VContainer>
            ...
            >
              <VRow>
                ...
                <VCol cols="12">
                  <AppTextField
                    v-model="editedItem.folder_drive"
                    :label="$t('Label.Folder-Path')"
                    :placeholder="$t('Label.Folder-Path')"
                  />
                </VCol>
              </VRow>
            </VForm>
