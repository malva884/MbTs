...
const loadItems = async () => {
  ...
  const { data: resultData } = await useApi<any>(createUrl('/workflow/office/', {
    query: {
      page: page.value,
      itemsPerPage: itemsPerPage.value,
      sortBy: sortBy.value,
      orderBy: orderBy.value,
      ufficio: ufficioFilter.value,
      disabled: attivoFilter.value,
    },
  }))
...
const headers = computed(() => [
  { title: t('Label.Ufficio'), key: 'ufficio' },
  { title: t('Label.Descrizine'), key: 'descrizione' },
  { title: t('Label.Disattivo'), key: 'disattivo' },
  { title: 'ACTIONS', key: 'actions', sortable: false },
])

const save = async () => {
  if (editedItem.value.ufficio) {
    let path = '/workflow/office/store/'
    if (editedItem.value.id)
      path = `/workflow/office/update/${editedItem.value.id}`
...
<template>
  ...
  >
    <AppCardActions
      ...
      :title="editedItem.id ? `${$t('Label.Modifica')} Ufficio` : `${$t('Label.Nuovo')} Ufficio`"
      no-actions
    >
      <VCard>
        <VCardText>
          <VContainer>
            ...
            >
              <VRow>
                ...
                <VCol cols="12">
                  <AppTextField
                    v-model="editedItem.ufficio"
                    :rules="[requiredValidator]"
                    :label="$t('Label.Ufficio')"
                    :placeholder="$t('Label.Ufficio')"
                  />
                </VCol>
