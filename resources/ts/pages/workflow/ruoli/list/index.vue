...
const loadItems = async () => {
  ...
  const { data: resultData } = await useApi<any>(createUrl('/workflow/ruoli/', {
    query: {
      page: page.value,
      itemsPerPage: itemsPerPage.value,
      sortBy: sortBy.value,
      orderBy: orderBy.value,
      ruolo: ruoloFilter.value,
...
const save = async () => {
  if (editedItem.value.role && editedItem.value.model) {
    let path = '/workflow/ruoli/store/'
    if (editedItem.value.id)
      path = `/workflow/ruoli/update/${editedItem.value.id}`
