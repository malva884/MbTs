...
const loadItems = async () => {
  ...
  const { data: resultData } = await useApi<any>(createUrl('task/dashboard/aggiornati', {
    query: {
      page: page.value,
...
watch(props, () => {
  if (props.aggiornaTask)
    loadItems()
})
</script>

<template>
  <VCard :title="$t('Label.Task-Aggiornati')">
    <template #append>
      <div class="me-n2">
