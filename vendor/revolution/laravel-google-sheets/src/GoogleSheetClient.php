...
class GoogleSheetClient
{
    ...
    protected function useAssertCredentials(string $userEmail = ''): bool
    {
        ...
        $serviceCredentialsJson = Arr::get($this->config, 'service.credentials_json', '');

        // Try credentials_json first (for environment variables)
        if (!empty($serviceCredentialsJson)) {
            $this->client->setAuthConfig(json_decode($serviceCredentialsJson, true));
